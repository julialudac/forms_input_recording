...
def test_save_filled_form():
    ...
    with unittest.mock.patch("builtins.input", return_value=SAME_ANSWER):
        ...
        actual_form_str = etree.tostring(form_with_answers.getroot()).decode('utf-8')
...
def test_create_answers_structure_from_similar_filled_forms():
   ...
   actual_answers_structure_root = etree.fromstring(etree.tostring(actual_answers_structure.getroot()).decode('utf8'))
   ...
   that work."""
   expected_answers_structure_root_str = etree.tostring(expected_answers_structure.getroot()).decode() 
